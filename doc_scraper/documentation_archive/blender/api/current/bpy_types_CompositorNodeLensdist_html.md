...
scraped_at: 2025-09-08T14:23:08.694708
title: CompositorNodeLensdist(CompositorNode)¶
---
...
_class _bpy.types.CompositorNodeLensdist(_CompositorNode_)¶
...
distortion_type¶
  ...
  * `RADIAL` Radial – Radially distorts the image to create a barrel or a Pincushion distortion.

  * `HORIZONTAL` Horizontal – Horizontally distorts the image to create a channel/color shifting effect.

Type:

    

enum in [`RADIAL`, `HORIZONTAL`], default `RADIAL`

use_fit¶

    

For positive distortion factor only: scale image such that black areas are not
visible. (Deprecated: Use Fit input instead.)
...
Enable/disable jittering (faster, but also noisier). (Deprecated: Use Jitter
input instead.)
...
only). (Deprecated: Use distortion_type property instead.)
